...
def canonicalize_laserscan(
    ...
    *,
    ...
    Sequential input groups are minimum-pooled using valid samples only.
    """
    if expected_raw_beams <= 0 or canonical_beams <= 0:
        raise ValueError("beam counts must be positive")
    if expected_raw_beams % canonical_beams != 0:
        raise ValueError("expected_raw_beams must be divisible by canonical_beams")
    ...
    pooled_range = np.min(np.where(valid, grouped, range_max), axis=1)
    normalized_range = np.clip(pooled_range / range_max, 0.0, 1.0).astype(np.float32)
