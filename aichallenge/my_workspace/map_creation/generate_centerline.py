...
def smooth_points_default(points: np.ndarray) -> np.ndarray:
    if len(points) < 7:
        return points
    ...
    # Use circular wrap mode to avoid artifacts at the start/end of closed loops.
...
def run(args: argparse.Namespace) -> None:
    ...
    is_closed = True
    points_px = smooth_points_default(points_px)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=float,
        default=0.0,
        help="Optional expected lap length in meters. When >0, closed contours are filtered by +/-15%%.",
    )
    p.add_argument(
        "--allow-any-length",
        action="store_true",
