#!/usr/bin/env python3
"""
Global raceline generation from a centerline CSV.

The output follows the common F1TENTH raceline layout:
  s_m; x_m; y_m; psi_rad; kappa_radpm; vx_mps; ax_mps2

The default path uses the bundled global trajectory optimizer and expects its
Python dependencies to be installed in the runtime environment.
"""
...
def generate_global_opt_raceline(
    ...
    if opt_type not in {"shortest_path", "mincurv", "mincurv_iqp"}:
        raise RuntimeError("Global optimizer supports shortest_path, mincurv, and mincurv_iqp.")
    ...
    try:
        ...
        message = (
            "Global optimizer requires trajectory-planning-helpers and quadprog. "
...
def generate_with_global_optimizer(
    ...
    return generate_global_opt_raceline(
...
def run(args: argparse.Namespace) -> None:
    ...
    for direction, centerline, widths, direction_out_path in direction_plan:
        raceline = generate_with_global_optimizer(args, centerline, widths)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(description="Generate a global raceline from a centerline CSV.")
