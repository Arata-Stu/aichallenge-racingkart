...
Install and verify the global raceline optimizer dependencies.
...
run_check() {
    ...
    set +e
    "${PYTHON_BIN}" - "${OPTIMIZER_ROOT}" >"${CHECK_LOG_FILE}" 2>&1 <<'PY'
import importlib
import os
import sys

optimizer_root = os.path.abspath(sys.argv[1])
if not os.path.isdir(optimizer_root):
    raise SystemExit(f"optimizer root not found: {optimizer_root}")
...
print("[global-opt] Dependency check passed")
PY
    local status=$?
    set -e
    cat "${CHECK_LOG_FILE}"
...
install_default_stack() {
    "${PIP_BIN}" install -r "${OPTIMIZER_ROOT}/requirements.txt"
}
